...
import type { PlannerItem, ToolId } from "../model/types";
import type {
  CreatePartContext,
  PartDefinition,
  PartRegistry,
...
export function createPartRegistry(
  ...
  for (const definition of definitions) {
    ...
    if (byId.has(id)) throw new Error(`Duplicate part definition: ${id}`);
  ...
  const catalog = Object.freeze(definitions.map(({ catalog: entry }) => entry));
  ...
  return Object.freeze({
    ...
    create(id: string, context: CreatePartContext) {
      ...
      return {
        ...
        catalogId: id,
        name: definition.catalog.name,
        x: context.snapToGrid(context.desk.width / 2 - width / 2, partGrid),
        y: context.snapToGrid(context.desk.depth / 2 - height / 2, partGrid),
        width,
        height,
        rotation: 0,
